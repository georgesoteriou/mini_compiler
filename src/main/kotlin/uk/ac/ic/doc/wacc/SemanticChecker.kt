...
class SemanticChecker {
    lateinit var currStatement: Statement

    private fun errorLog(it: Statement) {
        when (it) {
            is Statement.Block ->
                println("Error within Block starting at line ${it.location.lineNum}. ${it.context}")
            is Statement.While ->
                println("Error within While starting at line ${it.location.lineNum}. ${it.context}")
            is Statement.If ->
                println("Error within If starting at line ${it.location.lineNum}. ${it.context}")
            is Statement.Print ->
                println("Error in print at line ${it.location.lineNum}. ${it.context}")
            is Statement.PrintLn ->
                println("Error in println at line ${it.location.lineNum}. ${it.context}")
            is Statement.Exit ->
                println("Error in exit at line ${it.location.lineNum}. ${it.context}")
            is Statement.Return ->
                println("Error in return at line ${it.location.lineNum}. ${it.context}")
            is Statement.FreeVariable ->
                println("Error in free at line ${it.location.lineNum}. ${it.context}")
            is Statement.ReadInput ->
                println("Error in read at line ${it.location.lineNum}. ${it.context}")
            is Statement.VariableAssignment ->
                println("Error in assignment at line ${it.location.lineNum}. ${it.context}")
            is Statement.VariableDeclaration ->
                println("Error in declaration at line ${it.location.lineNum}. ${it.context}")
            is Statement.Skip ->
                println("Error in skip at line ${it.location.lineNum}. ${it.context}")
        }
    }
    ...
    private fun checkStatement(statement: Statement, activeScope: ActiveScope, returnType: Type): Boolean {
        ...
        return when (statement) {
            ...
            is Statement.VariableAssignment -> {
                ...
                if (rhs is Expression.CallFunction) {
                    val funType = activeScope.findType(rhs.name).orElse(Type.TError)
                            as? Type.TFunction ?: return false
    ...
    private fun exprType(expr: Expression, activeScope: ActiveScope): Type {
            ...
            is Expression.CallFunction -> {
                ...
                when (type) {
                    is Type.TFunction -> type.returnType
                        ...
                        currStatement.context = "Function does not exist"
                        Type.TError
                    }
                }
            }
                ...
                val type = if (!arrayElemType.isEmpty()) {
                    arrayElemType.foldRight(arrayElemType[0]) { type, acc ->
                        if (Type.compare(type, acc)) {
                            ...
                            currStatement.context = "array type missmatch"
                            return Type.TError
                        }
                    }
                } else {
                        ...
                        is Type.TArray -> {
                            ...
                            if (it is Expression.Literal.LInt) {
                                if (it.int >= size || it.int < 0) {
                                    currStatement.context = "Array index [${it.int}] out of bounds"
                                    return Type.TError
                                }
                            }
